...
#         # Define the architecture of the EEG encoder here
#         pass

#     def forward(self, eeg_input):
#         # Process the EEG input through the model
#         pass

#     def encode(self, eeg_input):
#         # Generate the vector representation of the EEG input
#         pass
...
from .utils import load_pretrained_weights, create_mlp
from safetensors.torch import load_file

import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
#from video_encoder.utils import create_mlp
...
class TemporalConv(nn.Module):
    ...
    def forward(self, x, **kwargs):
        x = rearrange(x, 'B N A T -> B (N A) T')# [1. 64, 4, 200]-[1, 256, 200]
        B, NA, T = x.shape
        x = x.unsqueeze(1)
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
                 ...
                 use_mean_pooling=True, init_scale=0.001, mlp_layers=[512, 1024], mlp_dropout=None, **kwargs):
        super().__init__()
        ...
        # we change the head here to a mlp layer
        self.head = create_mlp(input_dim=embed_dim, hidden_dims=mlp_layers[:-1], output_dim=mlp_layers[-1], dropout=mlp_dropout) if num_classes > 0 else nn.Identity()
        ...
        if isinstance(self.fc_norm, nn.LayerNorm):
            nn.init.ones_(self.fc_norm.weight)  # Initialize weights to 1
            nn.init.zeros_(self.fc_norm.bias)  # Initialize biases to 0
    ...
    def load_ckpts(self, config, fine_tuned, project_dir):
        assert type(fine_tuned) == bool, "fine_tuned should be a bool"
        assert "ckpt_load_path" in config.keys(), "ckpt_load_path should be in config"
        
        if not fine_tuned:
            if config["ckpt_load_path"]:
                ckpt_path = os.path.join(project_dir, config["ckpt_load_path"])
                assert os.path.exists(ckpt_path), f"Checkpoint file not found at {ckpt_path}"
                self, missing_keys, unexpected_keys = load_pretrained_weights(model=self, ckpt_path=ckpt_path)
        else:
            if config["ckpt_load_path"]:
                ckpt_path = os.path.join(project_dir, config["ckpt_load_path"])
                assert os.path.exists(ckpt_path), f"Checkpoint file not found at {ckpt_path}"
                ...
                print(f"===> Loaded eeg encoder checkpoint from {config['ckpt_load_path']}")
                if missing_keys:
...
# Usage 
@register_model
def labram_base_patch200_200(pretrained=False, config=None, **kwargs):
    assert config is not None, "config should not be None"
    mlp_layers = config["mlp_layers"]
    mlp_dropout = config["dropout"]
    model = NeuralTransformer(
        patch_size=200, num_classes=1024, embed_dim=200, depth=12, num_heads=10, mlp_ratio=4, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), init_values=0.1, mlp_layers=mlp_layers, mlp_dropout=mlp_dropout, **kwargs)
    model.default_cfg = _cfg()
    
    return model

@register_model
def labram_base_patch200_200_no_head(pretrained=False, config=None, **kwargs):
    model = NeuralTransformer(
        patch_size=200, num_classes=-1, embed_dim=200, depth=12, num_heads=10, mlp_ratio=4, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), init_values=0.1, **kwargs)
    model.default_cfg = _cfg()
    
    return model

# from model import labram_base_patch200_200_2  # Assuming this includes LaBraM model definitions
# model = labram_base_patch200_200_2(pretrained=True)