...
class VADHead(DETRHead):
    ...
    def forward(self,
        ...
        brain_key_pos = torch.nn.Parameter(torch.randn(1, 1, self.eeg_dim).to('cuda'))
        brian_query_pos = torch.nn.Parameter(torch.randn(1, 1, 256).to('cuda'))
        brain_q_with_pos = ego_map_query + brian_query_pos
        brain_k_with_pos = brain_feats + brain_key_pos
        linear_key = torch.nn.Linear(self.eeg_dim, 256).to('cuda')    
        linear_value = torch.nn.Linear(self.eeg_dim, 256).to('cuda')  
        brain_key = linear_key(brain_k_with_pos)    
        brain_value = linear_value(brain_feats) 
        key_padding_mask = torch.zeros(1, 1, dtype=torch.bool).to('cuda')
        # print(brain_q_with_pos.shape)
        # print(brain_key.shape)
        # print(brain_feats.shape)
        # breakpoint()
