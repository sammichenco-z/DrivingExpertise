...
class ContrastiveModel(nn.Module):
    def __init__(self, config):
        ...
        config["eeg_encoder"]["dropout"] = self.dropout
        config["video_encoder"]["dropout"] = self.dropout
        self.temperature = nn.Parameter(torch.tensor(float(config["temperature"]), dtype=torch.float32), requires_grad=False) # learnable temperature parameter
        self.eeg_encoder = timm.create_model("labram_base_patch200_200", pretrained=True, config=config["eeg_encoder"])
        ...
        if self.config["fine_tuning_policy"] == "missing_trainable":
            ...
            for name, param in self.video_encoder.named_parameters():
                if name not in self.video_encoder_missing_keys:
                    ...
                    param.requires_grad = True
        elif self.config["fine_tuning_policy"] == "all_trainable":
            for name, param in self.eeg_encoder.named_parameters():
                param.requires_grad = True
            ...
            raise ValueError(f"Unknown fine-tuning policy: {self.config['fine_tuning_policy']}")
        
        return self.eeg_encoder_missing_keys, self.eeg_encoder_unexpected_keys, self.video_encoder_missing_keys, self.video_encoder_unexpected_keys

    def change_fine_tuning_policy(self, policy="all_trainable"):
        if policy == "missing_trainable":
            for name, param in self.eeg_encoder.named_parameters():
                if name not in self.eeg_encoder_missing_keys:
                    param.requires_grad = False
                else:
            ...
            raise ValueError(f"Unknown fine-tuning policy: {policy}")

    def check_parameters(self, logger):
        # summary the parameters that are trainable
        logger.info("Trainable parameters in EEG encoder:")
        ...
        logger.info("Trainable parameters in Video encoder:")
        ...
        logger.info(f"Total trainable parameters: {eeg_trainable_params}+{video_trainable_params}={total_trainable_params}")
    ...
    def contrastive_loss(self, eeg_representation, video_representation):
        ...
        similarity_matrix = similarity_matrix * torch.exp(self.temperature) # (N, N)
    ...
    def save_epoch_ckpts(self, epoch):
        ...
        temperature_ckpt_path = os.path.join(ckpt_save_dir, f"temperature_epoch_{epoch}.safetensors")
        ...
        save_file({"temperature": self.temperature}, temperature_ckpt_path)
        ...
        print(f"Saved temperature parameter to {temperature_ckpt_path}, temperature={self.temperature}")
    ...
    def remove_old_epoch_ckpts(self, num_ckpts_to_keep):
        ...
        # for both eeg encoders and video encoders and temperature
        num_ckpts_to_keep *= 3
    ...
    def save_final_ckpts(self):
        ...
        temperature_ckpt_path = os.path.join(self.project_dir, self.config["temperature_ckpt_path"])
        ...
        print(f"Saved EEG encoder checkpoint to {eeg_encoder_ckpt_path}")
        print(f"Saved Video encoder checkpoint to {video_encoder_ckpt_path}")
        print(f"Saved temperature parameter to {temperature_ckpt_path}")