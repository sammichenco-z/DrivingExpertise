...
def train_model_ddp(epochs, batch_size, learning_rate, weight_decay):
    ...
    val_dataset_config = config["data"].copy()
    val_dataset_config["subject_type"] = "all"
    val_dataset = RealCarDataset(val_dataset_config, "val")
...
def initialize_parameters(arg):
    ...
    version_identifier = f"duration_{config['duration']}_fps_{config['fps']}_{arg.subject_type}_+eeg_{eeg_area_str}+{config['model']['extra_identifier']}"
    config["version_identifier"] = version_identifier
    config["data"]["dataset_dir"] = os.path.join(config["data"]["dataset_dir"], f"processed_duration_{config['duration']}_video_fps_{config['fps']}")
    config["data"]["subject_type"] = arg.subject_type
    config["data"]["eeg_area"] = arg.eeg_area
    config["data"]["data_duration"] = float(config["duration"])
